  ...
  src/
    ...
    sim/                   # PURE. No DOM/Canvas/Audio/Math.random.
      ...
      systems/
        ...
        collision.js       # broadphase + shield-ellipse / hull-polygon tests
 ...
 *  @property {(c, rx, ry, opts) => void} drawEllipse  // shields (concentric) [ROC-DMG-3]
 ...
 *  @property {number} [shield]    // remaining ellipses [ROC-DMG-3]
...
8. **collision** — broadphase grid; shield-ellipse vs hull-polygon tests. `[ROC-DMG-1,5]`
9. **damage** — apply hits: shield first (flash ellipse), then hull (flash white + fragments + smoke/fire); destruction → bounty, drops, particles. `[ROC-DMG-2,6,6a,7; ROC-ECO-1]`
...
- **Shields:** N concentric `drawEllipse` rings = remaining strength; brief brighten on `shieldFlashTtl`. `[ROC-DMG-2,3]`
...
- **Shielded target:** collision shape is an **ellipse** `(rx,ry)` sized to the hull; projectile point/segment-in-ellipse test. `[ROC-DMG-1]`
- **Unshielded target:** collision shape is the **2D convex silhouette polygon** of the hull (precomputed per mesh); segment-vs-polygon test. (Chosen over circle-clusters for fidelity to the visible shape; cheap at these counts — see Open Decisions.) `[ROC-DMG-5]`
