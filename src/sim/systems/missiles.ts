...
export function missilesSystem(world: World, dt: number, cfg: MissilesConfig = DEFAULT_MISSILES): void {
  ...
  // either docking approach, and while the wreck is waiting to respawn. [ROC-MIS-1,7,8,9,12,
  // ROC-DCKG-2, ROC-MDCK-1, ROC-LIFE-3]
  if (
    p.missileGrade > 0 &&
    world.levelState !== 'DOCKING' &&
    world.levelState !== 'MID_DOCKING' &&
    !world.player.respawnPending &&
    enemyOnScreen(world)
  ) {
